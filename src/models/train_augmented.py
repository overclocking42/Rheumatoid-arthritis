#!/usr/bin/env python3
"""
M4 Mac-Optimized Training with Augmentation
Fast training with WeightedRandomSampler + Focal Loss + MixUp
"""
...
def build_model(arch):
    if arch == "efficientnet":
        ...
        model.classifier = nn.Sequential(nn.Dropout(0.2), nn.Linear(in_f, 1))
        return model
    elif arch == "vit":
        model = models.vit_b_16(weights=models.ViT_B_16_Weights.IMAGENET1K_V1)
        in_f = model.heads.head.in_features
        model.heads.head = nn.Sequential(
            nn.LayerNorm(in_f), nn.Dropout(0.15),
            nn.Linear(in_f, 256), nn.GELU(), nn.Dropout(0.1),
            nn.Linear(256, 1)
        )
        return model
    else:
...
if __name__ == "__main__":
    print("="*70)
    print("M4 GPU-OPTIMIZED TRAINING WITH AUGMENTATION")
    print("="*70)
    
    results = {}
    for arch in ["efficientnet", "vit", "resnet"]:
        results[arch] = train_model(arch, max_epochs=30)
